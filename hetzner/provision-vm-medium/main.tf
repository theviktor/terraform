...
locals {
  ...
  web_monitoring_private_ip = cidrhost(var.vpc_cidr, 200)
...
resource "hcloud_firewall" "web_monitoring" {
  name = "${var.name_prefix}-fw-web-monitoring"
...
# SBC firewall
resource "hcloud_firewall" "sbc" {
  name = "${var.name_prefix}-fw-sbc"
  ...
  rule {
    ...
    protocol   = "tcp"
    port       = "5060"
    source_ips = [var.allowed_sbc_cidr]
    description = "SIP TCP"
  }
